 ...
 *     indicate that there are no parameters or that the parameters all use the default format
 *     (text); or one, in which case the specified format code is applied to all parameters; or it
 ...
 *   Int32
 *     The length of the parameter value, in bytes (this count does not include itself). Can be
 *     zero. As a special case, -1 indicates a NULL parameter value. No value bytes follow in the
 *     NULL case.
 *   Byten
 ...
 *     indicate that there are no result columns or that the result columns should all use the
 *     default format (text); or one, in which case the specified format code is applied to all
 *     result columns (if any); or it can equal the actual number of result columns of the query.
 *   Int16[R]
...
enum class FormatType(val code: Int) {
  ...
  companion object {
    fun valueOf(code: Int): FormatType = values()[code]
  }
}
